...
// Ask Replicate for classic URL outputs instead of FileOutput streams.
// This is simpler and more reliable for a Next.js JSON response.
...
async function outputToDataUrl(value: unknown): Promise<string | null> {
  ...
  console.log('Generated output downloaded:', buffer.length, 'bytes', contentType);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Replicate output received. Array:', Array.isArray(output), 'Type:', typeof output);
    ...
    if (!generatedImage) {
      console.error('Replicate returned an output but no HTTP image URL could be extracted:', output);
    ...
    return NextResponse.json(
      { error: message, errorKind: status === 401 ? 'AUTHENTICATION' : 'PROVIDER_ERROR' },
      { status },
    );
  }
}
