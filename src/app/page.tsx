...
export default function Home() {
  ...
  return <main className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-emerald-50/30 text-slate-900">
    ...
    {showLibrary && <div className="fixed inset-0 z-50 bg-slate-950/40 p-3 sm:p-8" onClick={() => setShowLibrary(false)}><aside onClick={e => e.stopPropagation()} className="ml-auto flex h-full w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-200 p-5"><div><h2 className="text-xl font-bold">📚 Biblioteca</h2><p className="text-sm text-slate-500">Tus imágenes quedan guardadas en este dispositivo.</p></div><button onClick={() => setShowLibrary(false)} className="rounded-full bg-slate-100 px-3 py-2">✕</button></div><div className="grid flex-1 grid-cols-2 gap-3 overflow-y-auto p-4 sm:grid-cols-3">{library.length === 0 ? <div className="col-span-full py-20 text-center text-slate-500">Todavía no hay imágenes guardadas.</div> : library.map(item => <div key={item.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50"><button onClick={() => selectLibraryImage(item)} className="block w-full"><img src={item.image} alt={item.prompt} className="aspect-square w-full object-cover" /></button><div className="p-2"><div className="truncate text-xs text-slate-600">{item.prompt}</div><div className="mt-2 flex justify-between gap-2"><button onClick={() => selectLibraryImage(item)} className="text-xs font-semibold text-emerald-700">Usar</button><button onClick={() => removeLibraryImage(item.id)} className="text-xs text-red-500">Borrar</button></div></div></div>)}</div></aside></div>}
  </main>;
}
